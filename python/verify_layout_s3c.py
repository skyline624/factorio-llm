"""Validation LIVE S3c : placement beacons + modules insérés + solver bonus appliqué.

Pré-requis : serveur Factorio 2.0 headless lancé (scripts/start_factorio_dedicated.bat)
avec le mod factorio-llm chargé (version S3b : describe/measure beacon + PRODUCER_TYPES
étendu + STARTING_ITEMS beacon/modules/electric-furnace), RCON 127.0.0.1:27015
(pw "factoriollm"). Pas de relance requise pour S3c (modifs Python uniquement).

Chaîne de test : iron-plate@10/s via electric-furnace (smelting) + 4 beacons speed-module-3.
  ...
  Bonus solveur (Option A, compute_module_effect formule 2.0 FFF#409) :
    4 beacons speed-module-3 -> dist*sqrt(n)*mod = 1.5*sqrt(4)*0.5 = 1.5 speed_bonus.
    effective_speed = 2*(1+1.5) = 5.0 ; per_machine = 1*1*5.0/3.2 = 1.5625.
    count = ceil(10/1.5625) = 7 (vs 16 sans bonus -> bonus appliqué, machine_count réduit).

8 recs :
  1. set_test_mode + setup (kit S3b beacon/modules/electric-furnace).
  2. solve iron-plate@10 + electric-furnace + module_effects -> feasibility ok.
  3. machine_count réduit par bonus (7 electric-furnaces vs 16 sans bonus).
  4. plan() beacons_per_stage=4 -> totals beacon>0 + electric-furnace>0.
  5. plan() chaque beacon a modules=["speed-module-3","speed-module-3"] (LayoutEntity).
  6. can_place chaîne beaconnée 0 collision (filtre frontier+water, pattern S2b-3).
  7. LIVE place 1 beacon + insert 2 speed-module-3 + scan_factory -> get_module_inventory.
  8. back-compat : beacons_per_stage=0 (défaut) -> 0 beacon (layout S2 inchangé).

Lancement :
    cd python
    python verify_layout_s3c.py
"""
...
def can_place_all(api, lp, rcon, label, v_frontier):
    """can_place non destructif sur chaque entity (filtre frontier + water, pattern S2b-3)."""
...
def main() -> int:
    ...
    rec("S3c-1 : set_test_mode + setup (kit beacon/modules/electric-furnace)",
    ...
    # --- Rec 2 : solve iron-plate@10 + electric-furnace + module_effects -> feasibility ok ---
    # 4 beacons speed-module-3 -> compute_module_effect(4, "speed-module-3") :
    # formule 2.0 FFF#409 = dist*sqrt(n)*mod = 1.5*sqrt(4)*0.5 = 1.5 speed_bonus.
    meff = compute_module_effect(4, "speed-module-3")
    ...
    rec("S3c-2 : solve iron-plate@10 + electric-furnace + 4 beacons (module_effects)",
        splan.feasibility == "ok",
        f"feasibility={splan.feasibility} speed_bonus={meff.speed_bonus}")

    # --- Rec 3 : machine_count réduit par bonus (7 vs 16 sans bonus) ---
    # Sans bonus : per_machine = 1*2/3.2 = 0.625 -> count=ceil(10/0.625)=16.
    # Avec 4 beacons (speed_bonus=1.5) : effective_speed=2*2.5=5.0 -> per_machine=5.0/3.2=1.5625
    # -> count=ceil(10/1.5625)=7.
    ...
    rec("S3c-3 : machine_count réduit par bonus (7 electric-furnaces vs 16 sans bonus)",
        ok_count,
        f"count={ef_node.machine_count if ef_node else '?'} speed_bonus={ef_node.speed_bonus if ef_node else '?'}")

    # --- Rec 4 : plan() beacons_per_stage=4 -> totals beacon>0 + electric-furnace>0 ---
    ...
    t = lp.totals
    rec("S3c-4 : plan totals beacon>0 + electric-furnace>0",
        lp.feasibility == "ok" and t.get("beacon", 0) > 0 and t.get("electric-furnace", 0) > 0,
        f"feas={lp.feasibility} beacon={t.get('beacon',0)} ef={t.get('electric-furnace',0)}")

    # --- Rec 5 : chaque beacon a modules=["speed-module-3","speed-module-3"] ---
    beacons = [e for e in lp.entities if e.role == "beacon"]
    ok_mods = len(beacons) > 0 and all(
        e.modules == ["speed-module-3", "speed-module-3"] for e in beacons)
    rec("S3c-5 : chaque LayoutEntity beacon a modules=[speed-module-3]*2",
        ok_mods, f"n_beacons={len(beacons)} modules[0]={beacons[0].modules if beacons else '?'}")

    # --- Rec 6 : can_place chaîne beaconnée 0 collision ---
    v_frontier = (bbox["y1"] + bbox["y2"]) / 2.0 + 60.0
    ok_n, fail_n, hors_map, obs_terr = can_place_all(api, lp, rcon, "iron-plate+beacons", v_frontier)
    rec("S3c-6 : can_place chaîne beaconnée 0 collision (filtre frontier+water)",
        fail_n == 0,
    ...
    # Place un beacon à une position claire, insert 2 modules, scan_factory doit le retourner
    # avec get_module_inventory = [{name:speed-module-3, count:2}] (modules insérés).
    bx, by = 60.0, 60.0
    # cleanup préalable
    rcon.query_lua(
        "local s=game.surfaces['nauvis'] or game.surfaces[1]; "
        "for _,e in ipairs(s.find_entities_filtered{name='beacon',area={{59,59},{61,61}}}) do e.destroy() end"
    )
    ...
    rec("S3c-7 : LIVE beacon + insert 2 speed-module-3 + scan_factory get_module_inventory",
    ...
    # --- Rec 8 : back-compat beacons_per_stage=0 (défaut) -> 0 beacon ---
    ...
    req_nob = LayoutRequest(plan=splan_nob, terrain=terrain, anchor=anchor, facing=2,
                            constraints=LayoutConstraints())  # défaut beacons_per_stage=0
    lp_nob = plan(req_nob, geo)
    beacons_nob = [e for e in lp_nob.entities if e.role == "beacon"]
    rec("S3c-8 : back-compat beacons_per_stage=0 -> 0 beacon (layout S2 inchangé)",
