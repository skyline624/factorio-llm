...
| **S4d** (mod Lua `M.generate_terrain` + `control.lua` fl_tools + wrapper `ModApi.generate_terrain(x,y,radius=30)` + 3 tests `test_mod_api`) | 9/9 unit + 8/8 live | ✅ **Résout le CONSTAT S1d/S1g out-of-map**. `surface.request_to_generate_chunks({x,y}, r_chunks)` + `surface.force_generate_chunk_requests()` (API Factorio 2.0 — l'investigation S1g la croyait inexistante). Non-destructif (cree du terrain vierge). `radius` tuiles (cap 200) → `r_chunks=floor(r/32)+1`, retourne `{x,y,radius_chunks,generated,total}`. Sans ceci, `walk_to` (pathfinding natif) ne peut pas planifier vers du out-of-map (tuiles non walkable) → ornière ; `generate_terrain` génère AVANT le walk → cible walkable. **Live `verify_walk_gen.py` 8/8** : rec 6 probe=(2000,-2000) avant `None` (chunk non généré lève erreur, PAS "out-of-map") → après `deepwater` gen=25/25 = génère du VRAI out-of-map ; rec 7-8 walk_to par étapes (generate-then-walk) mode prod `walking_state` marche RÉELLE 245 tuiles au sud (pas teleport, refusé en prod) = cas d'usage P2 (corridor de chunks devant le character puis l'y envoie). **Back-compat stricte** : additions pures, 67 pytest/177 runner inchangés. |
...
5. **S4b n'adresse PAS le CONSTAT S1d headless** (débordement u hors starting_area = artefact) ; il adresse les obstacles RÉELS scannés (water, rochers) dans la zone constructible (CONSTAT S2a). Le CONSTAT S1d/S1g (out-of-map) est résolu par **S4d** (`generate_terrain` via `request_to_generate_chunks` + `force_generate_chunk_requests`).
...
- **Out-of-map headless** : FactoryBuilder ne génère pas la map par défaut. `scan_obstacles` retourne vide hors starting_area ; `scan_tiles_bbox` retourne "out-of-map" → `Terrain.tile_grid` peuplé → planner détecte. **Résolu par S4d** : `ModApi.generate_terrain(x, y, radius)` génère synchrone les chunks (API 2.0 `request_to_generate_chunks` + `force_generate_chunk_requests`) avant un `walk_to` ou un build au-delà de la starting_area.
...
**S4 COMPLET (a+b+c+d)** — clôt la roadmap LayoutPlanner (S0→S4). S4d (extension post-roadmap) résout le CONSTAT S1d/S1g out-of-map via `generate_terrain` et valide le cas d'usage P2 (generate-then-walk, déplacement du character au-delà de la starting_area). Suite logique : relier solveur+layout → FactoryBuilder (Coordinator émet `ProductionRequest` + zone/gisement, FactoryBuilder arbitre tiers puis délègue) = couche P2 `agents-roadmap`.
